...
<div class="{{ $widthClass }}">
    ...
    {{-- Visible label --}}
    @if ($label)
        <label for="{{ $inputId }}" class="block mb-1 font-semibold text-gray-700 dark:text-white">
            {{ $label }}
        </label>
    @else
        {{-- Label hidden for accessibility only --}}
    ...
    <div class="relative">
        <select id="{{ $inputId }}" name="{{ $inputName }}" aria-invalid="{{ $hasError ? 'true' : 'false' }}"
            ...
            {{ $attributes->merge([
                'class' =>
                    "appearance-none px-4 py-2 pr-10 my-1 rounded-lg transition-all duration-300 ease-in-out transform $styles $widthClass " .
