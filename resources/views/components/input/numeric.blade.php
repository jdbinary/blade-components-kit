...
<div class="{{ $widthClass }}">
    ...
    <div class="relative">
        <input type="number" id="{{ $inputId }}" name="{{ $inputName }}" placeholder="{{ $placeholder }}"
            value="{{ $inputValue }}" aria-label="{{ $label ?: $placeholder }}"
            aria-invalid="{{ $hasError ? 'true' : 'false' }}"
            aria-describedby="{{ $hasError ? 'error-' . $inputId : null }}"
            {{ $attributes->merge([
                'class' =>
                    "px-4 py-2 pr-7 my-1 rounded-lg transition-all duration-300 ease-in-out transform $styles $widthClass " .
