...
@php
    ...
    $inputId = $name ?: 'radio-' . str_pad(mt_rand(0, 9999), 4, '0', STR_PAD_LEFT);
    $inputName = $name ?: $inputId;
    $groupId = 'group-' . $inputId;

    $styles = $colorStyles[$color] ?? $colorStyles['default'];
    $selectedValue = old($inputName, $selected);
...
<div class="space-y-4">
    ...
    >
        @foreach ($options as $value => $text)
            @php $id = $inputName . '_' . $loop->index; @endphp
    ...
    @if ($helperText && !$hasError)
        <p id="helper-{{ $groupId }}" class="text-sm text-gray-500 dark:text-gray-400 ">
            {{ $helperText }}
        </p>
    @endif
